...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!trim($d['ad'] ?? '')) {
        ...
        // Mükerrer telefon kontrolü (çift kayıt önleme)
        if ($telefon !== '') {
            $mk = $pdo->prepare("SELECT id, ad, soyad, firma_adi FROM musteriler WHERE telefon IN (?,?) OR telefon2 IN (?,?) LIMIT 1");
            $mk->execute([$telefon, $d['telefon'] ?? '', $telefon, $d['telefon'] ?? '']);
            if ($mevcut = $mk->fetch()) {
                $hata = 'Bu telefon numarası zaten kayıtlı: <a href="detay.php?id=' . (int)$mevcut['id'] . '">'
